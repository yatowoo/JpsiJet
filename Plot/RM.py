...
for i in range(3):
  response.GetAxis(3).SetRangeUser(JET_PT_BINS[i],JET_PT_BINS[i+1])
  SUM = None
  for j in range(3):
    response.GetAxis(2).SetRangeUser(JET_PT_BINS[j],JET_PT_BINS[j+1])
    RM[i][j] = response.Projection(0,1)
