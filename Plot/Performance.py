...
def DrawQA_Electron(output):
  ...
  c.SetMargin(0.16, 0.02, 0.12, 0.02)
  ...
  CaloQA['MB']['E'].GetXaxis().SetLabelSize(0.05)
  CaloQA['MB']['E'].GetYaxis().SetRangeUser(1e-9, 20)
  CaloQA['MB']['E'].GetYaxis().SetTitleOffset(1.45)
  CaloQA['MB']['E'].GetYaxis().SetTitleSize(0.05)
  CaloQA['MB']['E'].GetYaxis().SetLabelSize(0.05)
  ROOT.gPad.SetLogy()
  CaloQA['MB']['E'].Draw('PE')
...
def DrawQA_InvMass(tagInfo, JPSI_PT_CUT_LOW = 10., JPSI_PT_CUT_UP = 35., JET_PT_CUT_LOW = 10., JET_PT_CUT_UP = 35.):
  ...
  PAVE_CUTS.AddText('   %.0f < #it{p}_{T,e^{+}e^{-}} < %.0f GeV/#it{c}' % (JPSI_PT_CUT_LOW, JPSI_PT_CUT_UP) )
  PAVE_CUTS.AddText('|#it{#eta}_{jet}| < 0.5')
  PAVE_CUTS.AddText('    %.0f < #it{p}_{T,jet} < %.0f GeV/#it{c}' % (JET_PT_CUT_LOW, JET_PT_CUT_UP) )
  PAVE_CUTS.Draw('same')
  # Label
...
def DrawQA_Eff(mc):
  ...
  hP.GetYaxis().SetTitleOffset(1.0)
